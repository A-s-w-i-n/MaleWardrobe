...
const userwishlist = async function (req, res, next) {
  try {
    ...
    let wishlistDisplay = await wishlistinfo
      .aggregate([
        ...
        },
        {
          $project: {
            ...
            wishlistProductView: { $arrayElemAt: ["$wishlistProducts", 0] },
          },
        },
      ])
      .toArray();
...
const usercart = async function (req, res, next) {
  try {
    ...
    // if (user) {
    let productCart = await cartInfo
      .aggregate([
        {
          $match: { user: user },
        },
...
const usercheckout = async function (req, res, next) {
  try {
    ...
    if (user) {
      let userCheck = await addressinfo.aggregate([
        { $match: { user: user } },
        { $unwind: "$address" },
      ]);
...
const userloginval = async function (req, res, next) {
  try {
    ...
    let userloginfo = await userinfo.findOne({
      name: userloginvalidation.name,
    });

    let userstatus = await userinfo.findOne({
      name: userloginfo.name,
      status: false,
    });
...
const productDetaile = async function (req, res, next) {
  try {
    ...
    const reviewProduct = await reviewinfo.find({
      productId: reviewProuctFindId,
    });
...
const cartManage = async function (req, res, next) {
  try {
    ...
    if (usersession == null) {
      ...
      if (userCart) {
        let proExist = await cartInfo.findOne({
          $and: [{ user: userId }, { "product.item": proId }],
        });
    ...
    for (i = 0; i < productCart.length; i++) {
      if (productCart[i]["item"] == cartItem.product) {
        productCart[i]["quantity"] =
          productCart[i]["quantity"] + cartItem.count;
      }
    }

    const cartQauntity = await cartInfo.findOneAndUpdate(
      { $and: [{ user: user }, { "product.item": cartItem.product }] },
  ...
  try {
    ...
    if (userExt == null) {
      await addressinfo.insertMany([
        { user: req.session.user.name, address: details },
      ]);
    } else {
      await addressinfo.updateOne(
        { user: user },
...
const savedAddress = async function (req, res, next) {
  try {
    ...
    let passAdd = await addressinfo.aggregate([
      { $match: { user: user } },
      { $unwind: "$address" },
      { $match: { "address.id": passId2 } },
    ]);
...
const proceed = async function (req, res, next) {
  try {
    ...
    let status =
      req.body.paymentmethod === "COD" ? "orderConfirmed" : "orderConfirmed";
    ...
    (payment = req.body.paymentmethod), (grand_Total = paymentTotal);
...
const userOrder = async function (req, res, next) {
  try {
    ...
    const userOrders = await orderinfo.aggregate([
      { $match: { orderedUser: orderId } },
      { $sort: { _id: -1 } },
    ]);
...
const extAddressEdit = async function (req, res, next) {
  try {
    ...
    const adressEditExt = await addressinfo.aggregate([
      { $match: { user: user } },
      { $unwind: "$address" },
      { $match: { "address.id": addressEdit } },
    ]);
...
const addressUpdate = async function (req, res, next) {
  try {
    ...
    await addressinfo.updateOne(
      { user: user, address: { $elemMatch: { id: editUserAddress } } },
      {
        $set: {
...
const applyCoupon = async function (req, res, next) {
  try {
    ...
    let usedCouponCheck = await userinfo.find({
      name: user,
      usedCoupon: { $in: [couponId] },
    });
    ...
    if (user) {
      ...
      if (hmac == raz["payment[razorpay_signature]"]) {
        ...
        for (i = 0; i < products[0].product.length; i++) {
          order.products[0].product[i].paymentId =
            req.body["payment[razorpay_payment_id]"];
        }
        await orderinfo.insertMany([orders]);
        await orderinfo.updateOne(
          { user: req.session.user },
          { $set: { userUsedCoupon: userCoupon } }
        );
        await cartInfo.deleteOne({ user: req.session.user.name });
...
const userProductOrder = async function (req, res, next) {
  try {
    ...
    let userOrderDelivered = await orderinfo.find({
      orderedUser: orderId,
      _id: new ObjectId(orderedId),
    });

    let userOrderDeliveredId = userOrderDelivered.orderStatus;
    const productUserData = await orderinfo.aggregate([
      { $unwind: "$products" },
      { $match: { orderedUser: orderId } },
      { $project: { product: "$products.product" } },
      { $match: { _id: new ObjectId(orderedId) } },
    ]);

    res.render("userOrderDetails", {
      productUserData,
      userOrderDeliveredId,
      userOrderDelivered,
    });
  } catch (error) {
    next();
...
const priceSortOne = async function (req, res, next) {
  try {
    const priceSortOne = await productinfo.find({
...
const priceSortTwo = async function (req, res, next) {
  try {
    const priceSortTwo = await productinfo.find({
...
const priceSortThree = async function (req, res, next) {
  try {
    const priceSortThree = await productinfo.find({
...
const changePage = async function (req, res, next) {
  try {
    ...
    const shopProducts = await productinfo
      .find()
...
const returnUser = async function (req, res, next) {
  try {
    ...
    await orderinfo.updateOne(
      { _id: new ObjectId(userReturnId) },
      { $set: { orderStatus: "returnRequsted" } }
    );
    await orderinfo.updateOne(
      { _id: new ObjectId(userReturnId) },
      { $set: { returnDate: returnDate } }
    );
    const addReturnField = await orderinfo.updateOne(
      { _id: new ObjectId(userReturnId) },
      { $set: { newReturnStats: "returnRequsted" } }
    );
...
const cancelOrder = async function (req, res, next) {
  ...
  if (payment == "onlinepayment" || payment == "wallet") {
    await orderinfo.updateOne(
      { _id: new ObjectId(cancelOrederId) },
      { $set: { orderStatus: "OrderCanceled" } }
    );
    await orderinfo.updateOne(
      { _id: new ObjectId(cancelOrederId) },
      { $set: { cancelStatus: "OrderCanceled" } }
    );
  ...
  try {
    ...
    if (userWishlist) {
      ...
      if (wishlistExt == null) {
        ...
        let wishlistCheck = await wishlistinfo
          .find({
            $and: [
              { user: user },
              { "wishlistProduct.productId": wishlistProductId },
        ...
        if (wishlistCheck.length == 0) {
          await wishlistinfo.updateOne(
            { user: user },
            { $push: { wishlistProduct: { productId: wishlistProductId } } }
          );
        }
      }
...
const wishProductDel = async function (req, res, next) {
  try {
    ...
    await wishlistinfo.updateOne(
      { user: user },
      { $pull: { wishlistProduct: { productId: wishDelId } } }
    );
...
const userReview = async function (req, res, next) {
  try {
    ...
    const userReview = await reviewinfo.insertMany([
      { review: submitReview, user: user, productId: search },
    ]);
...
const userWalletHistory = async function (req, res, next) {
  try {
    ...
    const walletDatas = await orderinfo.find({
      orderedUser: user,
